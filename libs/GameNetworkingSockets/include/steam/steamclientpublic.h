...
public:
	...
	//-----------------------------------------------------------------------------
	void FullSet( vuint64 ulIdentifier, EUniverse eUniverse, EAccountType eAccountType )
	{
	...
	// Input  : ulSteamID -		64-bit representation of a Steam ID
	//-----------------------------------------------------------------------------
	void SetFromUint64( vuint64 ulSteamID )
	{
	...
	//-----------------------------------------------------------------------------
	vuint64 GetStaticAccountKey() const
	{
		// note we do NOT include the account instance (which is a dynamic property) in the static account key
		return (vuint64) ( ( ( (vuint64) m_steamid.m_comp.m_EUniverse ) << 56 ) + ((vuint64) m_steamid.m_comp.m_EAccountType << 52 ) + m_steamid.m_comp.m_unAccountID );
	}
	...
	static const char * Render( vuint64 ulSteamID );	// static method to render a uint64 representation of a steam ID to a string
...
public:
	...
	explicit CGameID( vuint64 ulGameID )
	{
		m_ulGameID = ulGameID;
	}
#ifdef INT64_DIFFERENT_FROM_INT64_T
	...
	static const char *Render( vuint64 ulGameID );		// static method to render a uint64 representation of a Game ID to a string

	vuint64 ToUint64() const
	{
		return m_ulGameID;
	}

	vuint64 *GetUint64Ptr()
	{
		return &m_ulGameID;
	}

	void Set( vuint64 ulGameID )
	{
		m_ulGameID = ulGameID;
	}
